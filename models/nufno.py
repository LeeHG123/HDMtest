...
# === Dual wrapper: choose NUDFT (original) or KNO integral ===
class DualSpectralConv1D(nn.Module):
    def __init__(self,
                 in_channels: int,
                 out_channels: int,
                 n_modes: int,
                 *,
                 temb_dim: int = 256,
                 # NUDFT-side args (kept for signature compatibility; passed to original class)
                 separable: bool = True,
                 bias: bool = True,
                 kappa_warm_start_scale: float = 0.3,
                 enable_band_gating: bool = True,
                 band_split_fracs: Tuple[float, float] = (0.4, 0.8),
                 band_fixed_edges: Optional[Tuple[float, float]] = None,
                 enable_time_freq_gate: bool = True,
                 time_gate_tau0_frac: float = 0.15,
                 time_gate_alpha_init: float = 8.0,
                 time_gate_hidden: int = 128,
                 ...
                 gabor_omega_init: float = 0.25,
                 # KNO
                 kernel_mode: str = "nufft",                # 'nufft' | 'kno'
                 kernel_type: str = "gsm",
                 kernel_Q: int = 2,
                 ...
                 measure_scale: float = 1.0):
        super().__init__()
        km = str(kernel_mode).strip().lower()
        if km == "nufft":
            self.impl = NUSpectralConv1D(
                ...
                bias=bias,
                temb_dim=temb_dim,
                kappa_warm_start_scale=kappa_warm_start_scale,
                enable_band_gating=enable_band_gating,
                band_split_fracs=band_split_fracs,
                band_fixed_edges=band_fixed_edges,
                enable_time_freq_gate=enable_time_freq_gate,
                time_gate_tau0_frac=time_gate_tau0_frac,
                time_gate_alpha_init=time_gate_alpha_init,
                time_gate_hidden=time_gate_hidden,
                enable_gabor_window=enable_gabor_window,
                gabor_omega_init=gabor_omega_init,
            )

        elif km == "kno":
            self.impl = KNOSpectralIntegral1D(
                in_channels=in_channels,
                out_channels=out_channels,
                temb_dim=temb_dim,
                kernel_type=str(kernel_type).lower(),
                kernel_Q=int(kernel_Q),
                kernel_x_hidden=int(kernel_hidden),
                kernel_time_hidden=int(kernel_time_hidden),
                enable_kernel_time_cond=bool(enable_kernel_time_cond),
                bias=bias,
                measure_scale=float(measure_scale),
            )
    ...
    def __getattr__(self, name):
        try:
            ...
            impl = self.__dict__.get("impl", None)
            if impl is not None and hasattr(impl, name):
                return getattr(impl, name)
            raise           

    def forward(self, x_feat: torch.Tensor, z: torch.Tensor, temb: Optional[torch.Tensor]) -> torch.Tensor:
        return self.impl(x_feat, z, temb)
...
class NUFNO(nn.Module):
    ...
    def __init__(self, config):
        ...
        kno_mode    = str(getattr(cfg, "kernel_mode", "nufft")).lower()   # 'nufft' | 'kno'
        ...
        self.spectral_blocks = nn.ModuleList([
            DualSpectralConv1D(
                ...
                # NUDFT-side (kept from your current config)
                ...
                gabor_omega_init=float(getattr(cfg, "gabor_omega_init", 0.25)),
                # KNO-side
                kernel_mode=kno_mode,
                kernel_type=kno_type,
                kernel_Q=kno_Q,
                kernel_hidden=kno_hidden,
