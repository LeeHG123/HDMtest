...
class KNOScalarKernel1D(nn.Module):
    ...
    def __init__(self,
        ...
        if self.kind == "rbf":
            ...
            _ell0 = torch.tensor(float(rbf_len_init) if rbf_len_init is not None else 0.25, dtype=torch.float32)
            self.log_len  = nn.Parameter(_inv_softplus(_ell0))
        elif self.kind == "gsm":
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, temb: Optional[torch.Tensor]) -> torch.Tensor:
        ...
        if self.kind == "rbf":
            ...
            diff2 = (x_ - y_)**2
            K = gain.view(B if gain.shape[0]==B else 1, 1, 1) * torch.exp(- diff2 / (ell.view(B if ell.shape[0]==B else 1,1,1)**2))
...
class MultiHeadKNOSpectralIntegral1D(nn.Module):
    """
    Multi-head 버전 KNO spectral integral layer.

    - 입력:  x_feat (B, C_in, N), z (B, N) with z = pi * x_norm, temb (B, C_temb)
    - 채널을 num_heads개 head로 나누고, 각 head마다 독립적인 KNOScalarKernel1D를 사용해
      full kernel K^{(h)}(x_i, x_j)를 적분한 뒤, head를 concat + 1x1 conv 로 채널 믹싱.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # ---- time embedding -> channel shift (MHLKNO 스타일과 동일) ----
        self.tact = nn.SiLU()
        self.tproj = nn.Linear(temb_dim, in_channels)
        self.tproj.weight.data = default_init()(self.tproj.weight.data.shape)
        nn.init.zeros_(self.tproj.bias)
        ...
        # ---- channel projections ----
        # in_proj:  (B, C_in, N) -> (B, C_in, N)
        # out_proj: (B, C_in, N) -> (B, C_out, N)
        ...
        # ---- full KNO kernel per head ----
        # share_kernel=True: 모든 head가 하나의 kernel 파라미터를 공유
        # share_kernel=False: head마다 독립적인 kernel 파라미터 (진짜 multi-kernel 구조)
    ...
    def forward(self, x_feat: torch.Tensor, z: torch.Tensor, temb: torch.Tensor) -> torch.Tensor:
        ...
        # 1) time embedding shift
        #    shift: (B, C),  x_feat <- x_feat + shift[:, :, None]
        shift = self.tproj(self.tact(temb))           # (B, C_in)
        x_feat = x_feat + shift.unsqueeze(-1)         # (B, C_in, N)

        # 2) value projection + head reshape
        v = self.in_proj(x_feat)                      # (B, C_in, N)
        v = v.view(B, self.num_heads, self.head_dim, N)  # (B, H, D_h, N)

        # 3) coordinates & trapz weights
        x_norm = z / math.pi                          # (B, N) in [-1, 1]
        w = _trapz_weights_1d(x_norm) * self.measure_scale  # (B, N)
        v_weighted = v * w.view(B, 1, 1, N)           # (B, H, D_h, N)

        # 4) head-wise full kernel integrals
        ...
        for h in range(self.num_heads):
            ...
            # y_h[b, d, i] = sum_j K[b, i, j] * v_h[b, d, j]
            # (B, N, N) × (B, D_h, N) -> (B, D_h, N)
            y_h = torch.einsum("bmn,bcn->bcm", K, v_h)
            y_heads[:, h, :, :] = y_h

        # 5) concat heads + output projection
        y_cat = y_heads.view(B, C, N)                 # (B, C_in, N)
        out = self.out_proj(y_cat)                    # (B, C_out, N)
        if self.bias is not None:
            out = out + self.bias
        return out        
...
class KNO(nn.Module):
    ...
    def __init__(self, config):
        ...
        # --- NEW: multi-head kernel 옵션 ---
        #   - num_kernel_heads == 1  → 기존 single-head KNO
        #   - num_kernel_heads > 1   → MultiHeadKNOSpectralIntegral1D 사용
        ...
        if self.hidden_channels % self.num_heads != 0:
            raise ValueError(
                f"hidden_channels ({self.hidden_channels}) must be divisible by "
                f"num_kernel_heads ({self.num_heads})"
            )
        ...
        # 좌표 스케일 (HilbertDiffusion에서 coord_scale로 설정)
        measure_scale = float(getattr(cfg, "measure_scale", 1.0))
        ...
        # --- 스펙트럴 블록 선택: single-head vs multi-head ---
        if self.num_heads == 1:
            # 기존 single-head KNO (완전 호환)
            self.spectral_blocks = nn.ModuleList([
            ...
            # Multi-head Full KNO: head마다 독립 kernel 파라미터
            self.spectral_blocks = nn.ModuleList([
                MultiHeadKNOSpectralIntegral1D(
                    ...
                    share_kernel_across_heads=False,   # 각 head마다 다른 GSM 파라미터
                    rbf_len_init=rbf_len_init,
                )
                for _ in range(self.n_layers)
            ])
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.n_layers):
            ...
            # single-head / multi-head 모두 동일 인터페이스
