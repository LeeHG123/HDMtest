...
class NUSpectralConv1D(nn.Module):
    ...
    def forward(self, x_feat: torch.Tensor, z: torch.Tensor, temb: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        Fr = torch.bmm(x_feat, cos) / max(N, 1)     # (B, C_in, M)
        Fi = torch.bmm(x_feat, -sin) / max(N, 1)    # (B, C_in, M)
        ...
        # ===== 밴드 게이팅 (역변환 직전) =====
        #   mask: (M,) → (1,1,M)로 브로드캐스트하여 실수/허수 성분에 동일 적용
        ...
        # Inverse NUFFT
        yr = torch.bmm(Gr, cos.transpose(1, 2))  # (B, C_out, N)
        yi = torch.bmm(Gi, sin.transpose(1, 2))  # (B, C_out, N)
        y = (yr - yi) / max(self.M, 1)
