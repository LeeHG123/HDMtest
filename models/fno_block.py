...
class FactorizedSpectralConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, n_modes, n_layers=1, scale='auto', separable=False,
        ...
        num_corners = 2**(self.order - 1)
        num_weights = num_corners * self.n_layers

        for i in range(num_weights):
            is_2d = (len(n_modes) == 2)
            
            # joint_factorization 설정에 따라 사용할 가중치를 선택합니다.
            if self.joint_factorization:
                # True이면 모든 레이어가 단일 공유 가중치 텐서를 사용합니다.
                current_weight = self.weight
            else:
                # False이면 ModuleList에서 각 레이어에 해당하는 가중치를 사용합니다.
                current_weight = self.weight[i]
            ...
            self.layer.append(
                get_contract_fun(
                    ...
                    current_weight, 
                    hidden=out_channels, 
                    temb_dim=256, # 또는 self.hidden_channels
                    act=nn.SiLU(), 
    ...
    def forward(self, y, indices=0):
        ...
        num_corners = len(list(itertools.product(*mode_indexing)))
        
        for i, boundaries in enumerate(itertools.product(*mode_indexing)):
            idx_tuple = [slice(None), slice(None)] + [slice(*b) for b in boundaries]

            layer_offset = indices * num_corners
            contract_fun_index = layer_offset + i
            
            out_fft[idx_tuple] = self.layer[contract_fun_index](x[idx_tuple], temb)
