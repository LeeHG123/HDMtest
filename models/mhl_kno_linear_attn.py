...
class LinearAttentionKNO1D(nn.Module):
    ...
    def forward(
        ...
        V_lin = V.permute(0, 1, 3, 2)                          # (B, H, N, D_h)

        # 4) Unified RF feature → Q, K
        phi = self._get_phi(x_coords, temb)                     # (B, N, H, D_k)
        phi = phi.permute(0, 2, 1, 3)                           # (B, H, N, D_k)
        Q = phi
        K = phi

        # 5) Linear Attention
        context = torch.einsum("bhnk,bhnv->bhkv", K, V_lin)     # (B, H, D_k, D_h)
        out_heads = torch.einsum("bhnk,bhkv->bhnv", Q, context) # (B, H, N, D_h)

        # 6) heads concat + out projection
        out_heads = out_heads.permute(0, 1, 3, 2).reshape(B, C, N)  # (B, C, N)
