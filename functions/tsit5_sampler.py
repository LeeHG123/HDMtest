...
from functions.utils import kernel 
# -----------------------------------------------------------------------------
...
def sample_probability_flow_ode(
    ...
    *,
    ...
    # --- Hilbert-noise covariance data ---
    hilbert_len: float = 0.04,        # config.data.hyp_len
    hilbert_gain: float = 1.0,        # config.data.hyp_gain
    hilbert_metric: str = "euclidean",
    coord_scale: float = 10.0,        # dataset.coord_scale
    coord_offset: float = 0.0,        # dataset.coord_offset
    ...
    # ---- precompute C^{-1} via Cholesky, per batch ----
    B, N = x_coord.shape
    I_cache = None
    Ls = []
    with torch.no_grad():
        # 정규화 좌표 -> 원 좌표
        x_raw = (x_coord * coord_scale + coord_offset).to(device)
        for b in range(B):
            K = kernel(x_raw[b].view(-1), x_raw[b].view(-1),
                    lens=hilbert_len, gain=hilbert_gain,
                    metric=hilbert_metric, device=device).to(torch.float32)
            if I_cache is None or I_cache.shape[0] != N:
                I_cache = torch.eye(N, device=device, dtype=K.dtype)
            L = torch.linalg.cholesky(K + cov_jitter * I_cache)
            Ls.append(L)

    def Kinv_apply(v: torch.Tensor) -> torch.Tensor:
        # v: (B, N) -> (B, N), compute C^{-1} v per batch via cholesky_solve
        out = torch.empty_like(v)
        for b in range(B):
            sol = torch.cholesky_solve(v[b].unsqueeze(1), Ls[b])  # (N,1)
            out[b] = sol.squeeze(1)
        return out    
    ...
    def reverse_f(s, y):
        ...
        score = Kinv_apply(model_out) / sigma_t[:, None]              # (B, N)
