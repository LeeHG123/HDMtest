...
def hilbert_loss_fn(
    ...
    *,
    global_step: Optional[int] = None,
    max_steps: Optional[int] = None,
    spec_lambda: float = 0.0,         # 스펙트럼 잔차 손실 계수(0이면 비활성)
) -> torch.Tensor:
    """
    ...
    # ----- 2) 스펙트럼 가중 잔차 손실(NUDFT) -----
    spec_loss = output.new_tensor(0.0)
    if spec_lambda > 0.0:
        pairs = _iter_kappas_with_meta(model)  # [(kappa_l, split_fracs_l), ...]
        if pairs:
            # 잔차와 위상은 한 번만 구성
            r = (output - target)                 # (B,N)
            z = math.pi * x_coord                 # (B,N)

            block_losses = []
            for kappa_l, split_fracs_l in pairs:
                kappa_l = kappa_l.to(r.device)    # (M_l,)

                # Forward NUDFT (블록별 κ에 대해 한 번씩)
                Fr, Fi = _forward_nudft_real(r, z, kappa_l)
                amp2   = Fr.pow(2) + Fi.pow(2)    # (B, M_l)

                # 블록별 밴드 스케줄 가중(평균이 1이 되도록 내부 정규화)
                w_spec_l = _band_weight_schedule(
                    kappa_l,
                    split_fracs=split_fracs_l,
                    global_step=global_step,
                    max_steps=max_steps,
                ).view(1, -1)                     # (1, M_l)

                # 모드 평균 → 배치 평균 → 스칼라
                block_loss = (amp2 * w_spec_l).mean()
                block_losses.append(block_loss)

            # 블록들 사이 평균(레이어 수에 대해 균등 가중)
            spec_loss = torch.stack(block_losses, dim=0).mean()

    return data_loss + spec_lambda * spec_loss
