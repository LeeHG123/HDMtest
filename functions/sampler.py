...
def sampler(x, model, sde, device, W, eps, dataset, steps=1000, sampler_input=None, method='srk'):
    """
    ...
    if method == 'srk':
        return sampler_srk(x, model, sde, device, W, eps, dataset, steps, sampler_input)
    else:
...
def sampler_srk(x, model, sde, device, W, eps, dataset, steps=1000, sampler_input=None):
    """
