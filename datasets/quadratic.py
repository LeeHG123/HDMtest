...
from datasets.adapted_grid import build_adapted_random_distribution_1d, sample_adapted_random_points_1d
...
class QuadraticDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, num_data: int, num_points: int, seed: int = 42,
                 grid_type: str = 'uniform', subgroups: int = 3, adapt_params: dict | None = None, noise_std: float = 0.0):
        super().__init__()
        ...
        self._adapt = {} if adapt_params is None else dict(adapt_params)        
        ...
        if grid_type == 'uniform':
            ...
            ap = {} if adapt_params is None else dict(adapt_params)
            self.subgroups = int(subgroups); self._adapt = ap
            def base_func(x): return x**2
            self._adapt_state = build_adapted_random_distribution_1d(
                x_min=-10.0, x_max=10.0, subgroups=self.subgroups, func=base_func,
                pilot_points=int(ap.get('pilot_points', 2048)),
                alpha=float(ap.get('alpha', 1.0)),
                smooth_sigma=float(ap.get('smooth_sigma', 3.0)),
                clip_quantile=float(ap.get('clip_quantile', 0.98)),
