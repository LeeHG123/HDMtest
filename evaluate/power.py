...
def _standardize(U):
    ...
    sig = U.std (1, keepdims=True) + 1e-8
    return (U - mu) / sig    

def _renormalize_pdf(U, dx):
    Z = (U.sum(1, keepdims=True) * dx)
    return U / Z    

def power_test(y_0, y_t, n_tests=100, n_perms=100, gamma=-1):
    # ① [B, N] 으로 변형
    X_raw = y_0.cpu().squeeze(-1)
    Y_raw = y_t.cpu().squeeze(-1)

    # ② pdf 재정규화 ― 격자 간격(Δx) 계산
    dx = 20.0 / (X_raw.shape[1] - 1)         # 도메인 [-10,10] 고정이면 이렇게
    X_pdf = _renormalize_pdf(X_raw, dx)
    Y_pdf = _renormalize_pdf(Y_raw, dx)

    # ③ 샘플별 z-score 표준화
    X = _standardize(X_pdf)
    Y = _standardize(Y_pdf)

    # 이하 기존 로직 동일 …
    n_tests = X.shape[0] // 10
    gamma = -1
    power = _power_test(X_samples=X, Y_samples=Y,
                        gamma=gamma, n_tests=n_tests,
                        n_perms=n_perms)
    return power * 100
