...
def plot_reject_null_hyp(X_samples, Y_samples, rejection):
    ...
    M = X_samples.shape[0]
    domain = np.linspace(-10, 10, X_samples.shape[1])
    for i in range(M):
...
def power_test(y_0, y_t, n_tests=None, n_perms=100, gamma=-1):
    X = y_0.detach().cpu().squeeze(-1)
    Y = y_t.detach().cpu().squeeze(-1)

    # 기본값: "테스트 하나에 10개 샘플" 규칙을 유지하되, 안전하게 처리
    if n_tests is None:
        group = 10
        n_tests = max(1, X.shape[0] // group)

    # X, Y 길이 불일치/나눠떨어지지 않음을 안전하게 보정
    total = min(X.shape[0], Y.shape[0])
    n_tests = min(n_tests, total)          # 과도한 n_tests 방지
    M = max(1, total // n_tests)           # 각 테스트당 샘플 수
    total_used = M * n_tests               # 사용 가능한 정합 길이
    X = X[:total_used]
    Y = Y[:total_used]

    return _power_test(X_samples=X, Y_samples=Y, gamma=gamma, n_tests=n_tests, n_perms=n_perms) * 100
