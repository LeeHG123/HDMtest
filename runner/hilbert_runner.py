...
class HilbertNoise:
    ...
    def visualize_basis_functions(self, save_path, num_to_plot=64):
        """
...
class HilbertDiffusion(object):
    def __init__(self, args, config, dataset, test_dataset, device=None):
        ...
        self.lambda_norm = float(getattr(config.training, "lambda_norm", 0.0))
        ...
        with torch.no_grad():
            for i, (x_fixed, y_fixed) in enumerate(val_loader):
                ...
                if not self.args.disable_resolution_free:
                    for N_res in res_free_points:
                        ...
                        if y_res_free is not None:
                            ...
                            loss_res_free = hilbert_loss_fn(model, self.sde, y_res_free, t, e, x_coord_norm_res_free, lambda_norm=self.lambda_norm)
                ...
                if calc_fixed_grid_loss:
                    ...
                    t = torch.rand(B, device=self.device) * (self.sde.T - self.sde.eps) + self.sde.eps
                    # Sample noise that matches the provided coordinates
                    e = self.W.free_sample(x_fixed_dev).to(self.device)
                    
                    loss_fixed = hilbert_loss_fn(model, self.sde, y_fixed_dev, t, e, x_coord_norm_fixed, lambda_norm=self.lambda_norm)
    ...
    def train(self):
        ...
        if config.model.model_type == "ddpm_mnist":
            ...
            model = NUFNO(config)            
        elif config.model.model_type == "NUNO_FNO_1D":
            from models.nuno_framework import NUFNO1D
            model = NUFNO1D(config)
        elif config.model.model_type == "NUNO_FNO_1D":
            from models.nuno_framework import NUFNO1D
            model = NUFNO1D(config)
        ...
        # Fixed-geometry precompute for NUNO
        if config.model.model_type == "NUNO_FNO_1D" and getattr(config.model, 'fixed_geometry', True):
            if hasattr(self.dataset, 'get_all_points'):
                try:
                    all_pts = self.dataset.get_all_points()
                except Exception:
                    all_pts = None
                if all_pts is not None:
                    try:
                        model.setup_decomposition(all_pts.squeeze(-1).to(self.device))
                        logging.info("Precomputed NUNO decomposition for fixed geometry.")
                    except Exception as e:
                        logging.warning(f"Failed to precompute NUNO decomposition: {e}")
        ...
        for epoch in range(config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                loss = hilbert_loss_fn(
                    model,
                    self.sde,
                    y,                        # (B, N) : 원신호 (x_0)
                    t,                        # (B,)   : 시간
                    e,                        # (B, N) : Hilbert noise
                    x_coord_norm,             # (B, N) : 좌표(랜덤 가능)
