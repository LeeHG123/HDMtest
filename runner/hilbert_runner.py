...
class HilbertDiffusion(object):
    def __init__(self, args, config, dataset, test_dataset, device=None):
        ...
        # --- subgroup 정보 로깅 ---
        self._log_subgroup_info(dataset, "train")
        self._log_subgroup_info(test_dataset, "val")        
        self.spec_lambda = float(getattr(config.training, "spec_lambda", 0.05))

    def _log_subgroup_info(self, ds, name: str):
        """
        학습/검증 데이터셋의 subgroup 정보를 콘솔 로그(+TensorBoard 텍스트)로 출력.
        ds.subgroup_meta가 있으면 그것을 사용, 없으면 x[0]으로 즉시 계산.
        """
        import logging
        try:
            grid_type = getattr(self.config.data, 'grid_type', 'uniform')
            G_cfg = int(getattr(self.config.data, 'subgroups', 3))
        except Exception:
            grid_type = 'uniform'
            G_cfg = 3

        meta = getattr(ds, 'subgroup_meta', None)

        if meta is None:
            # fallback: 첫 샘플 그리드로 즉석 집계
            x0 = ds.x[0]
            x_min = float(x0.min().item()); x_max = float(x0.max().item())
            bounds = torch.linspace(x_min, x_max, steps=G_cfg+1, device=x0.device)
            counts = torch.zeros(G_cfg, dtype=torch.long, device=x0.device)
            for g in range(G_cfg):
            ...
            meta = {"subgroups": G_cfg, "bounds": bounds.cpu(), "counts": counts.cpu()}

        G = int(meta["subgroups"])
        counts = meta["counts"].tolist()
        bounds = meta["bounds"].tolist()
        # logging
        logging.info(f"[{name}] grid_type={grid_type} | subgroups={G} | counts={counts}")
        # 사람이 보기 좋은 구간별 표기
        rng_str = ", ".join([f"[{bounds[i]:.6g},{bounds[i+1]:.6g}]{' ' if i<G-1 else ''}->{counts[i]}" for i in range(G)])
        logging.info(f"[{name}] subgroup ranges -> counts: {rng_str}")

        # TensorBoard 텍스트에도 기록(있을 경우)
        tb = getattr(self.config, "tb_logger", None)
        if tb is not None:
            lines = [f"grid_type={grid_type}, subgroups={G}", ""]
            for i in range(G):
                a = bounds[i]; b = bounds[i+1]; c = counts[i]
                lines.append(f"Group {i:02d}: [{a:.6g}, {b:.6g}] -> {c}")
            tb.add_text(f"subgroups/{name}", "\n".join(lines), global_step=0)        
    ...
    def train(self):
        ...
        if config.model.model_type == "ddpm_mnist":
            ...
            model = NUFNO(config)
        elif config.model.model_type == "TransformerHybridDESFNO":
            from models import TransformerHybridDESFNO
            model = TransformerHybridDESFNO(config)            
