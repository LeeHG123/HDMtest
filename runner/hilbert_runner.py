    ...
    def validate(self, model, val_loader, tb_logger, step):
        ...
        with torch.no_grad():
            for i, (x, y) in enumerate(val_loader):
                ...
                val_loss = hilbert_sobolev_loss_fn(model, self.sde, y, t, e, x_coord_norm, sobolev_weight=self.config.training.sobolev_weight).to(self.device)
    ...
    def train(self):
        ...
        for epoch in range(config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                loss_score = hilbert_sobolev_loss_fn(model, self.sde, y, t, e, x_coord_norm, sobolev_weight=config.training.sobolev_weight).to(self.device)
                # 정규화 보조-손실
