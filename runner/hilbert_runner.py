...
class HilbertNoise:
    ...
    def visualize_basis_functions(self, save_path, num_to_plot=100):
        """
...
class HilbertDiffusion(object):
        ...
        with torch.no_grad():
            for i, (x_fixed, y_fixed) in enumerate(val_loader):
                ...
                if not self.args.disable_resolution_free:
                    for N_res in res_free_points:
                        ...
                        if self.config.data.dataset == 'Quadratic':
                            ...
                            # x_res_free ∈ (0,1): 이미 상단에서 random으로 생성(0~1)하도록 처리하는 게 안전함.
                            # 만약 상단이 Quadratic 전용 범위(-10,10)로 되어 있다면 Doppler에서는 별도로 재정의:
                ...
                if calc_fixed_grid_loss:
                    ...
                    # (중요) y_fixed_dev를 원스케일로 되돌린 뒤, 훈련 통계로 재정규화
                    #   - y_fixed_dev는 test 정규화 → inverse_transform(test)로 원스케일
                    #   - 다시 train (mu, sigma)로 z-score 변환
                    ...
                    loss_fixed = hilbert_loss_fn(model, self.sde, y_fixed_train_norm, t, e, x_coord_norm_fixed)
    ...
    def train(self):
        ...
        # ------------------ (추가) learnable s 로깅 헬퍼 ------------------
        def _current_s(m, i: int | None = None) -> float | None:
            """
            DDP/단일모델 모두 지원.
            현재 주파수 스케일 s (또는 레이어별 s_i)를 float로 반환.
            """
            mm = m.module if hasattr(m, "module") else m
            if not getattr(mm, "learnable_freq_scale", False):
                return None
            import torch
            if getattr(mm, "per_layer_freq_scale", False):
                assert i is not None and 0 <= i < mm.n_layers
                z_logit = mm._z_logit[i]
                s = mm.s_min + (mm.s_max - mm.s_min) * torch.sigmoid(z_logit)
                return float(s.item())
            else:
                z_logit = mm._z_logit
                s = mm.s_min + (mm.s_max - mm.s_min) * torch.sigmoid(z_logit)
        ...
        for n, p in model.named_parameters():
            if not p.requires_grad:
                continue
            # learnable s 파라미터 (_z_logit)만 별도 그룹
            if "z_logit" in n:          # e.g. '_z_logit', 'module._z_logit.0', ...
        ...
        if scale_params:
            param_groups.append(
                {"params": scale_params, "lr": base_lr/5.0, "weight_decay": 0.0}
            )
        ...
        for epoch in range(config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                x_coord_norm = self._coord_norm(x)  # (B, N)
                ...
                # If coordinates are random per-sample, sample Hilbert noise aligned with x
                ...
                # 정규화 보조-손실 (좌표가 랜덤/비균등 간격인 경우까지 고려)
                if self.config.data.dataset in ["Gaussian"]:
                    # 원본 좌표로 가변 간격 Δx 계산: shape (B, N-1)
                    x_raw = x  # (B, N) 원본 좌표
                    dxs = (x_raw[:, 1:] - x_raw[:, :-1]).abs().to(self.device)

                    # t=0 근방에서의 모델 출력 사용 (보조항이므로 0으로 설정 유지)
                    t0 = torch.zeros(y.size(0), device=self.device)
                    model_input_t0 = torch.cat([y.unsqueeze(1), x_coord_norm.unsqueeze(1)], dim=1)  # (B, 2, N)
                    s_pred = model(model_input_t0, t0)  # 모델 출력 (epsilon 계열)

                    # 누적 적분으로 log p̂(x) 재구성 (C=0 고정), 가변 간격에 대해 사다리꼴 적분 사용
                    # mid = 0.5 * (s[i] + s[i+1]) * dx[i]
                    mid = 0.5 * (s_pred[:, 1:] + s_pred[:, :-1]) * dxs  # (B, N-1)
                    log_pdf_pred = torch.cat([torch.zeros(y.size(0), 1, device=self.device), torch.cumsum(mid, dim=1)], dim=1)  # (B, N)

                    # Ẑ ≈ ∫ exp(log p̂) dx  (사다리꼴 규칙)
                    f = torch.exp(log_pdf_pred)
                    Z_pred = (0.5 * (f[:, 1:] + f[:, :-1]) * dxs).sum(dim=1)  # (B,)
                ...
                # ------------------ (추가) learnable s 텐서보드 로깅 ------------------
                mm = model.module if hasattr(model, "module") else model
                if (step % 100 == 0) and getattr(mm, "learnable_freq_scale", False):
                    if getattr(mm, "per_layer_freq_scale", False):
                        for li in range(mm.n_layers):
                            val = _current_s(model, li)
                            if val is not None:
                                tb_logger.add_scalar(f"freq_scale/s_layer_{li}", val, global_step=step)
                    else:
                        val = _current_s(model)
                        if val is not None:
                            tb_logger.add_scalar("freq_scale/s_global", val, global_step=step)
                optimizer.zero_grad()
                loss.backward()
    ...
    def sample(self, score_model=None):
        ...
        if self.args.disable_resolution_free:
            ...
            if config.data.dataset == 'Quadratic':
                # ★ 기존: y00 = y_0 * 50.0  ->  Z-정규화 역변환으로 교체
            ...
            if config.data.dataset == 'Quadratic':
                ...
                # Ground-truth on original scale (비정규화된 실제 함수)
