    ...
    def train(self):
        ...
        for epoch in range(config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                loss_score = hilbert_sobolev_loss_fn(model, self.sde, y, t, e, x, sobolev_weight=config.training.sobolev_weight).to(self.device)
                # 정규화 보조-손실
                if self.config.data.dataset == "Gaussian":
                    ...
                    score = model(y, t0)                    # (B, N)  ∂/∂x log p̂(x)
        ...
        if self.args.sample_type in ["srk", "sde"]:        # SRK/Euler 케이스
            ...
            with torch.no_grad():
                ...
                y = unipc_sample_ode(
                    ...
                    x_coord=free_input.to(self.device),
                    device=self.device,
